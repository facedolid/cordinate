...
def load_images_from_directory():
    ...
    for file_name in os.listdir(UPLOAD_DIR):
        ...
        if not session.query(Image).filter_by(path=file_path).first():
            category = '未分類'  # Default category if not assigned
            new_image = Image(category=category.lower(), path=file_path)
            session.add(new_image)
    session.commit()

def check_dislike_exists(top_id, bottom_id, shoes_id, accessory_id):
    return session.query(Dislike).filter_by(
        top_id=top_id, bottom_id=bottom_id, shoes_id=shoes_id, accessory_id=accessory_id
    ).first() is not None
...
if page == '画像をアップロード':
    ...
    def get_images_by_category(category):
        return session.query(Image).filter_by(category=category.lower()).all()
    ...
    def is_disliked_combination(top, bottom, shoes, accessory):
        if top and bottom:
            dislike = session.query(Dislike).filter_by(
                top_id=top.id, bottom_id=bottom.id, shoes_id=shoes.id if shoes else None, accessory_id=accessory.id if accessory else None
            ).first()
    ...
    st.header('嫌いな組み合わせ')
    disliked_combinations = session.query(Dislike).all()
    ...
    st.header('好きな組み合わせ')
    favorite_combinations = session.query(Favorite).all()
